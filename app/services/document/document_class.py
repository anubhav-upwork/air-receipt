...
class DocumentClassService(BaseService[Document_Class, DocumentClass_Create, DocumentClass_Update]):
    def __init__(self, db_session: Session):
        super(DocumentClassService, self).__init__(Document_Class, db_session)

    def get_by_docclass(self, role: str) -> Optional[Document_Class]:
        return self.db_session.query(Document_Class).filter(Document_Class.doc_class == role).first()

    def create(self, obj_in: DocumentClass_Create) -> Document_Class:
        db_obj = User_Roles(
